...
$sql=SQL("select","count(n.id) as cant, n.tipo_nota,v.value_id,v.value_type,v.value,v.value_order,v.value_code
...
//Solo terminos aceptados == estado_id ='13'
 ...
 return SQL("select","if(terminosUP.id is not null,terminosUP.id_menor,tema.tema_id) as idTema,
 	tema.tema,
 	tema.isMetaTerm,
...
$limit=(secure_data($limit,"int")) ? $limit : "30";
...
return SQL("select","t.tema_id as tema_id,t.tema,t.isMetaTerm, i.indice,length(i.indice) as distancia
from $DBCFG[DBprefix]indice i,$DBCFG[DBprefix]tema t
...
return SQL("select","TT.tema_id as id,TT.tema,TT.isMetaTerm
			from $DBCFG[DBprefix]tema as TT
			left join $DBCFG[DBprefix]tabla_rel as no_menor on no_menor.id_menor=TT.tema_id
			left join $DBCFG[DBprefix]tabla_rel as no_mayor on no_mayor.id_mayor=TT.tema_id
			where
 ...
 $sqlTruncate=SQL("truncate","$DBCFG[DBprefix]indice");
  ...
  while($array=$sqlTerminosValidos->FetchRow()){
	...
	foreach($tema_ids_inverso as $tema_id){
  	...
  	echo $indice[$este_tema_id].$este_tema_id;
...
function SQLtermsNoBT($tesauro_id) 
{
	...
	return SQL("select","t.tema_id,t.tema,t.isMetaTerm
				from 
